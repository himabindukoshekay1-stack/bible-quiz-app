...
async function getBibleChapter(book, chapter) { const bibleId = process.env.NIV_BIBLE_ID; const apiKey = process.env.API_BIBLE_KEY; if (!bibleId) { throw new Error("Missing NIV_BIBLE_ID"); } if (!apiKey) { throw new Error("Missing API_BIBLE_KEY"); } console.log( "API KEY EXISTS:", !!apiKey ); console.log( "BIBLE ID:", bibleId ); // FETCH BOOKS const booksRes = await fetch( `https://rest.api.bible/v1/bibles/${bibleId}/books`, { headers: { "api-key": apiKey, "Content-Type": "text/plain", }, } ); console.log( "BOOKS STATUS:", booksRes.status ); const booksData = await booksRes.json(); if (!booksData.data) { console.log( "BOOKS RESPONSE:", booksData ); throw new Error( "Failed to fetch NIV books" ); } // FIND BOOK const matchedBook = booksData.data.find( (b) => b.name.toLowerCase() === book.toLowerCase() || b.nameLong.toLowerCase() === book.toLowerCase() ); if (!matchedBook) { throw new Error( `Book not found: ${book}` ); } console.log( "MATCHED BOOK:", matchedBook.name ); // FETCH CHAPTERS const chaptersRes = await fetch( `https://rest.api.bible/v1/bibles/${bibleId}/books/${matchedBook.id}/chapters`, { headers: { "api-key": apiKey, "Content-Type": "text/plain", }, } ); const chaptersData = await chaptersRes.json(); if (!chaptersData.data) { console.log( "CHAPTERS RESPONSE:", chaptersData ); throw new Error( "Failed to fetch chapters" ); } // FIND CHAPTER const matchedChapter = chaptersData.data.find( (c) => c.number === String(chapter) ); if (!matchedChapter) { throw new Error( `Chapter not found: ${chapter}` ); } console.log( "MATCHED CHAPTER:", matchedChapter.number ); // FETCH CHAPTER CONTENT const chapterRes = await fetch( `https://rest.api.bible/v1/bibles/${bibleId}/chapters/${matchedChapter.id}?content-type=text&include-notes=false&include-titles=false&include-chapter-numbers=false&include-verse-numbers=true`, { headers: { "api-key": apiKey, "Content-Type": "text/plain", }, } ); console.log( "CHAPTER STATUS:", chapterRes.status ); const chapterData = await chapterRes.json(); if ( !chapterData.data || !chapterData.data.content ) { console.log( "CHAPTER RESPONSE:", chapterData ); throw new Error( "Failed to fetch NIV chapter" ); } // PARSE VERSES const content = chapterData.data.content; const lines = content .split("\n") .map((line) => line.trim()) .filter(Boolean); const verses = []; for (const line of lines) { const match = line.match( /^(\d+)\s+(.*)$/ ); if (!match) continue; verses.push({ verse: match[1], text: match[2].trim(), }); } // FALLBACK if (verses.length === 0) { verses.push({ verse: "1", text: content, }); } console.log( "PARSED VERSES:", verses.length ); return { text: verses .map((v) => v.text) .join(" "), verses, }; }
